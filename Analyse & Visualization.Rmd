...
## Create new order for data
I want to have different arrangement for day_of_week and months. So that I can work easier later.
...
### Summary the data with tables.

Here, we are looking at the following things:
* The average riding duration for casual riders and members riders by the day of weeks and month.
* 


#### The summary of ride duration in mintues between casual riders and member riders. 
```{r}
...
For now, I only focus on the duration within a day. On the hand, I am going to removing anything under 1 mins, because it might cause by technical problems when the rider rented the bikes. Since it is not my interest in the case study and investigate is needed in the future. 
...
There are few observation we can see here:
There are less causal rider than member rider; while the causal rider is willing to ride twice amount than a member rider based on the average duration.
Also, the average and the median are not quite close to each other, which means they are not normally distributed.


#### Comparasion on riding durion in mintus for casual riders and member riders by month and week
...
week_duration <- total_trip_clean %>% 
  ...
  view()
```
For casual riders, the average range between 20mins to 27min;
while the members has a average range between 12mins to 14min.

Casual riders has nearly twice amount of duration than members.
...
There are nearly 4% of riders start their rental at Clark St(The most popular); while there are only 2% of riders start their rental at Wells St(second popular). There are more double amount of the difference.
...
First two is comparing the number of rides on day of week and monthly for casual riders and member riders.
```{r}
total_trip_clean %>%
  filter(duration_mins >1 & duration_mins < 1440) %>% 
  group_by(type_of_rider, day_of_week) %>%
...
As we can see, members tend to have a ride through out the week. On the other hand, causal riders tend to have a ride during weekend.
...
```
As we can see most rider tend to rent the bike between May to October due to the cold weather(below 10'C) months.(refer:https://seasonsyear.com/USA/Illinois/Chicago).



The third and the forth is comparing the average duration on day of week and monthly for casual riders and member riders.
```{r}
total_trip_clean %>%
  filter(duration_mins >1 & duration_mins < 1440) %>% 
  group_by(type_of_rider, day_of_week) %>%
...
```
On both graphs, we can clearly see that casual riders tend to rider longer than members during the whole year



Graph 5 and 6 show the the comparison of two difference type of bikes, electric bike or non electric bike.
```{r}
total_trip_clean %>%
  filter(duration_mins >1 & duration_mins < 1440) %>% 
...
```
Based on some investigation, with divvy's latest expansion on the west side of Chicago on last summer, more people were riding during the last summer.



The last two tables are showing the which station is their starting location.
V1 table is focusing on the exact name of the station name.
V2 table is focusing on the general street/area of the bike station.
```{r}
